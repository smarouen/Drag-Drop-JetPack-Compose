...
class DragDropListState (
    ...
    private val onMove: (Int, Int) -> Unit
        ...
        val elementDisplacement: Float?
            get() = currentIndexOfDraggedItem?.let { lazyListState.getVisibleItemInfoFor(absolute = it) }
...
fun onDragStart(offset: Offset){
        lazyListState.layoutInfo.visibleItemsInfo.firstOrNull(){ item -> offset.y.toInt() in item.offset..(item.offset + item.size) }?.also{
...
fun onDrag(offset : Offset){
    ...
    initialOffsets?.let{(topOffset, bottomOffset) ->
       ...
       currentElement?.let {
           hovered -> lazyListState.layoutInfo.visibleItemsInfo.filterNot { item -> item.offsetEnd < startOffset || item.offset > endOffset || hovered.index == item.index }
           .firstOrNull(){
               item -> val delta = startOffset - hovered.offset
               when{
               ...
               currentIndexOfDraggedItem?.let {
                   current -> onMove.invoke(current, item.index)
               }
